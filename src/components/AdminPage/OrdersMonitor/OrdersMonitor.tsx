...
const OrdersMonitor: React.FC = () => {
    ...
    const handleStatusChange = (created: number) => (e: React.ChangeEvent<{ name?: string | undefined; value: unknown; }>) => {
        // @ts-ignore
        setOrders({...orders, data: orders.data.map(o => (created === o.created ? {...o, status: (e.target.value as string)} : o))})
        db
            .collection('users')
            .doc(query)
            .set({
    ...
    return (
        <Accordion className={styles.orders}>
            ...
            <AccordionDetails className={styles.details}>
                ...
                {orders.data.reverse().map((o, i) => <OrderItem key={i} editMode handleStatusChange={handleStatusChange} order={o}/>)}
            </AccordionDetails>
        </Accordion>
    )
}
