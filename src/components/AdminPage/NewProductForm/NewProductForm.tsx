...
const NewProductForm = () => {
  ...
  const removeImageInput = (i: number) => (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    setProduct({
      ...product,
      images: [...product.images.filter((img, index) => index !== i)],
    })
  }
    ...
    <Accordion className={styles.form}>
      ...
      <AccordionDetails className={styles.form}>
        <form onSubmit={handleSubmit}>
          <TextField className={styles.input} required value={product.title} onChange={handleChange('title')} label="Title" />
          <div className={styles.multi}>
            {product.images.map((image, index) => (
              <div key={index} className={styles.formControl}>
                ...
                />
                {product.images[index] && <img className={styles.imgPreview} src={product.images[index]} alt={'Invalid source'} />}
                {index === 0 && (
                  <IconButton onClick={addImageInput} size={'small'}>
                    <AddRounded />
                  </IconButton>
                )}
                {index !== 0 && (
                  <div onClick={removeImageInput(index)}>
          ...
          <div className={styles.multi}>
            {product.specs.length === 0 && (
              <Button onClick={addSpecInput} color={'secondary'}>
                Add Specification
              </Button>
            )}
            {product.specs.map((spec, index) => (
              <div key={index} className={styles.formControl}>
                <div className={styles.specInput}>
                  ...
                  />
                  <TextField required name={String(index)} label={`Spec info`} value={product.specs[index].info} onChange={handleSpecChange(index, 'info')} />
                </div>
                {index === 0 && (
                  <IconButton onClick={addSpecInput} size={'small'}>
                    <AddRounded />
                  </IconButton>
                )}
                <div onClick={removeSpecInput(index)}>
                  <IconButton size={'small'}>
                    <RemoveRounded />
                  </IconButton>
                </div>
          ...
          <div className={styles.input}>
            <TextField value={product.price} onChange={handleChange('price')} required label="Price" />
          </div>
          <div className={styles.input}>
            <TextField value={product.rating} onChange={handleChange('rating')} required label="Rating" />
          </div>
          <div className={styles.input}>
            <TextField
              type={'number'}
