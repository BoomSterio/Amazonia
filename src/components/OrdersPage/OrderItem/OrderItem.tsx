...
const OrderItem: React.FC<Props> = ({ order, editMode = false, handleStatusChange }) => {
  ...
  const boughtItems = order.cart.map(item => (
    <CartProduct
      editable={false}
      animationDuration={0}
      key={item.id}
      ...
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMore />} aria-controls="panel1a-content" id="panel1a-header">
          <div className={styles.info}>
            <h4>
              Order #<span className={styles.id}>{order.created}</span>
            </h4>
            ...
            {editMode ? (
              <Select
                ...
                value={order.status}
                onChange={handleStatusChange ? handleStatusChange(order.created) : () => {}}
              >
        ...
        <AccordionDetails className={styles.details}>
          ...
          <div className={styles.additional}>
            <div>
              ...
              <p>
                {order.delivery.country}, {order.delivery.city}. {order.delivery.addressLine}
              </p>
